...
def mask_color(original_image):
    ...
    for (code, lower, upper) in boundaries:
        ...
        # morphology
        mask_open = cv2.morphologyEx(mask, cv2.MORPH_OPEN, kernelOpen, iterations=3)
        mask_close = cv2.morphologyEx(mask_open, cv2.MORPH_CLOSE, kernelClose, iterations=3)

        mask_final = mask_close

        ret, thr = cv2.threshold(mask_final, 127, 255, 0)
